...
**Status** shows whether access is on, that WireGuard is the one listener on the public interface and on which port, every Peer by name with its tunnel address and the date it was added, and each Exposed Service as the private address and port a connected device reaches it at — the dashboard always, the Memory UI while `sessions` is on — so what a device holding a config can reach is readable from `status` alone.

**What is exposed** is an allowlist, not a bind. The service drops all forwarding by default; the CLI writes `access/allowlist` from its Exposed Service list, and the entrypoint adds one DNAT rule per entry from the tunnel address to that service on the platform network. So "exposed" means "in the allowlist", and nothing becomes reachable by accident. The list holds the Orchestrator dashboard, at `http://10.13.13.1:8288` from a connected Peer, on every Target with access on; and the Memory UI, at `http://10.13.13.1:37777`, while `sessions` is on — the list follows the toggles, so turning `sessions` off with access on rewrites it and restarts the Access service, which drops live tunnels for a moment, and the UI stops being reachable from a Peer; turning `sessions` on adds it back. The Dispatch surface is never in it — it is keyless by design, and a test asserts the writer refuses it. A Peer's config routes only the tunnel subnet, so the device's other traffic is untouched. The rules resolve service addresses when the service starts, and every enable, DNS set-up and upgrade starts it afresh; if a service stops answering after you recreated its containers by hand, disable and enable access.

**The Memory UI is writable over Access through a proxy**, and the proxy is temporary by declaration ([ADR 0010](docs/adr/0010-the-platform-is-the-operators-agent-vps.md)). The claude-mem worker accepts requests only from loopback origins and offers no setting to change that; its viewer uses relative URLs, so over the VPN the page loads and every write is refused. So the Memory compose project runs a small nginx, `memory-ui`, beside the worker (`docker/memory/proxy.Dockerfile`), which rewrites the `Origin` header to a loopback value and forwards to the worker, and the allowlist names the proxy — never the worker, which a test asserts the writer refuses. The worker's admin routes still go by the request's own address, so a restart asked for from a Peer is refused, as it is from a Session. The condition for removing the proxy is written beside it in the compose file and here: an upstream claude-mem release that adds a configurable origin list. When that lands, the Exposed Service entry points at the worker again and the proxy goes.
